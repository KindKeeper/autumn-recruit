...
- 候选榜规则：有推进中岗位（已投递未终结）的**企业或集团**整企移出候选榜——按 `recruiting_group` 归组（同集团任一单位在推进，集团全部单位移出；无 group 的记录按 name 单企执行）；一个集团同时只允许一个单位在推进流程
- **招聘集团关联**：同一招聘系统/集团下的多个招聘单位，不合并记录，用 `recruiting_group` 字段关联（如"上海航天技术研究院（八院）"）；网申平台写 `portal` 字段（如 `sast.zhiye.com`）。"整企移出候选榜"规则按招聘单位（name）执行，不按 group——各单位流程相互独立。例外：投递时不选事业部、集团统一分配的（如腾讯/字节统一校招），可合并为一条集团记录，事业部写入"招聘主体"字段
